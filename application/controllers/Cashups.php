...
class Cashups extends Secure_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('cashups/manage2', $data);
	}
	...
	public function get_change($cashup_id = -1)
	{
		$in = $this->input->post();
		$open_date_formatter = date_create_from_format($this->config->item('dateformat') . ' ' . $this->config->item('timeformat'), $in['open_date'])->format('Y-m-d H:i:s');
		$close_date_formatter = date_create_from_format($this->config->item('dateformat') . ' ' . $this->config->item('timeformat'), $in['close_date'])->format('Y-m-d H:i:s');
		...
		// set the close date and time to the actual as this is a close session
		// ambil dari input
		$cash_ups_info->open_date = $open_date_formatter;
		$cash_ups_info->close_date = $close_date_formatter;
		$cash_ups_info->open_amount_cash = parse_decimals($in['open_amount_cash']);
		$cash_ups_info->transfer_amount_cash = parse_decimals($in['transfer_amount_cash']);
		...
		// parse currency
		$cash_amount_total = $cash_ups_info->closed_amount_total;
		// $cash_amount_total = $this->check_for_cash_rounding($cash_ups_info->closed_amount_total);

		$cash_ups_info->closed_amount_card = to_currency_no_money($cash_ups_info->closed_amount_card);
		$cash_ups_info->closed_amount_cash = to_currency_no_money($cash_ups_info->closed_amount_cash);
		$cash_ups_info->closed_amount_check = to_currency_no_money($cash_ups_info->closed_amount_check);
		$cash_ups_info->closed_amount_due = to_currency_no_money($cash_ups_info->closed_amount_due);
		$cash_ups_info->closed_amount_total = to_currency_no_money($cash_amount_total);
		$cash_ups_info->open_amount_cash = to_currency_no_money($cash_ups_info->open_amount_cash);
		$cash_ups_info->transfer_amount_cash = to_currency_no_money($cash_ups_info->transfer_amount_cash);



		die(json_encode(['status'=>$status,'result'=>$cash_ups_info]));
	}
	...
	*/
	...
	public function check_for_cash_rounding($total)
	{
		...
		$cash_rounding_code = $this->config->item('cash_rounding_code');
		$rounded_total = $total;
